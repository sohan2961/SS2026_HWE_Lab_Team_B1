...
architecture rtl of nexysA7_lane_rom_sobel_top is
    ...
    -- The ROM image is 320x240, but VGA is 640x480.               -- CHANGED (was 160x120)
    -- So x/2 and y/2 are used as ROM coordinates.                 -- CHANGED (was x/4, y/4)
    signal img_x : unsigned(8 downto 0);       -- 0 to 319         -- CHANGED (was 7 downto 0, 0 to 159)
    signal img_y : unsigned(7 downto 0);       -- 0 to 239         -- CHANGED (was 6 downto 0, 0 to 119)
...
begin
    ...
    -- Convert 640x480 screen coordinate to 320x240 image coordinate.   -- CHANGED
    -- Taking bits 9 downto 1 is the same as integer divide by 2.       -- CHANGED (was 9 downto 2, divide by 4)
    img_x <= x(9 downto 1) when visible = '1' else (others => '0');     -- CHANGED
    img_y <= y(8 downto 1) when visible = '1' else (others => '0');     -- CHANGED

    -- Image ROM: gives one grayscale pixel from the stored road image.
    rom_inst : entity work.image_rom_320x240                            -- CHANGED (was image_rom_160x120)
        port map (
            clk   => CLK100MHZ,
            ce    => pixel_ce,
            x     => img_x,
